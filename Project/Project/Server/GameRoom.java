...
public class GameRoom extends Room {
    ...
    //all new stuff by me past his point ************************************************************************************************************************************************************
    // Rock-Paper-Scissors game state
    private ConcurrentHashMap<Long, GameChoice> playerChoices = new ConcurrentHashMap<>();

    // Start the Rock-Paper-Scissors game
    public synchronized void startRockPaperScissors(ServerThread client, GameChoice choice) {
        if (currentPhase != Phase.READY) {
            client.sendMessage(Constants.DEFAULT_CLIENT_ID, "Can't start Rock-Paper-Scissors game at this time");
            return;
        }
        long playerId = client.getClientId();
        if (players.containsKey(playerId)) {
            // Ensure player is ready to participate
            if (!players.get(playerId).isReady()) {
                client.sendMessage(Constants.DEFAULT_CLIENT_ID, "You are not ready to participate");
                return;
            }

            // Store player's choice
            playerChoices.put(playerId, choice);
            System.out.println("Player " + playerId + " made choice: " + choice);

            // Check if all players have made their choices
            if (playerChoices.size() == numActivePlayers) {
                resolveRockPaperScissors();
            }
        }
    }

    // Resolve the Rock-Paper-Scissors game
    private synchronized void resolveRockPaperScissors() {
        List<GameChoice> choices = new ArrayList<>(playerChoices.values());
        int totalPlayers = choices.size();
        int[] counts = new int[GameChoice.values().length];

        // Count the choices
        for (GameChoice choice : choices) {
            counts[choice.ordinal()]++;
        }

        // Check for a draw
        if (counts[GameChoice.ROCK.ordinal()] == totalPlayers ||
            counts[GameChoice.PAPER.ordinal()] == totalPlayers ||
            counts[GameChoice.SCISSORS.ordinal()] == totalPlayers) {
            sendMessage(ServerConstants.FROM_ROOM, "It's a draw! Please play again.");
        } else {
            // Determine the winning choice
            int winningChoice = (counts[GameChoice.ROCK.ordinal()] == 0) ?
                                ((counts[GameChoice.PAPER.ordinal()] == 0) ? GameChoice.SCISSORS.ordinal() : GameChoice.PAPER.ordinal()) :
                                GameChoice.ROCK.ordinal();

            // Find the winners and losers
            List<Long> winners = new ArrayList<>();
            List<Long> losers = new ArrayList<>();
            for (long playerId : playerChoices.keySet()) {
                GameChoice choice = playerChoices.get(playerId);
                if (choice.ordinal() == winningChoice) {
